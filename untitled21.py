    ...
    if show_boxplots:
        ...
        if plot_type == "Gemiddelde Temperatuur":
            ...
            if forecast_checkbox_rides:
                ...
                future_days_rides = np.array([
                    (daily_rides['Day'].max() + pd.Timedelta(days=i) - daily_rides['Day'].min()).days
                    for i in range(1, 6)
                ]).reshape(-1, 1)
                ...
                fig.add_scatter(x=future_rides_data['Day'], y=future_rides_data['Number of Rides'], mode='markers+lines',
                                name='Voorspelde Ritten', line=dict(color='red', width=2))
            fig.update_traces(name='Aantal Ritten', line=dict(color='blue'))
